...
class MemoryEngine:
    ...
    def assemble_prompt(self, user_input: str) -> Tuple[str, MemoryTrace]:
        ...
        prompt_lines.extend([
            ...
            "7. 3-STEP ULTRA-TOKEN-EFFICIENT MEMORY PROTOCOL: You must evaluate every user message to determine if long-term memory retrieval is required. Calling search_memory(query) is MANDATORY whenever: (a) The user asks for holistic synthesis, personality analysis, global life reviews, or complete project portfolio overviews (short-term operational context is strictly INSUFFICIENT). (b) The user asks about past decisions, historical facts, or personal background not fully present in current context. (c) The user asks reflective questions about their activities, goals, or core identity. TO MAXIMIZE TOKEN EFFICIENCY: Step 1: Call search_memory(query) to receive up to 10 candidate document/entity hits. Step 2: Call get_document_outline(identifier) to inspect section headers without loading full text. Step 3: Call read_document_section(identifier, section_id) to retrieve ONLY the required section(s). NEVER summarize or judge the user's core personality based solely on temporary daily chores or short-term maintenance tasks."
        ])
