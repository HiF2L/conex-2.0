...
class MemoryEngine:
    ...
    def assemble_prompt(self, user_input: str) -> Tuple[str, MemoryTrace]:
        ...
        prompt_lines.extend([
            ...
            "7. MANDATORY TIER 3 MEMORY SEARCH PROTOCOL: You must evaluate every user message to determine if long-term memory retrieval is required. Calling search_memory(query) is MANDATORY whenever: (a) The user asks for holistic synthesis, personality analysis, global life reviews, or complete project portfolio overviews (short-term operational context is strictly INSUFFICIENT). (b) The user asks about past decisions, historical facts, or personal background not fully present in current context. (c) The user asks reflective questions about their activities, goals, or core identity. When searching, issue short, concise queries (e.g. search_memory('projects overview'), search_memory('identity core values')). NEVER summarize or judge the user's core personality based solely on temporary daily chores or short-term maintenance tasks."
        ])
