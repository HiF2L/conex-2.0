...
class MemoryEngine:
    ...
    def assemble_prompt(self, user_input: str) -> Tuple[str, MemoryTrace]:
        ...
        prompt_lines.extend([
            ...
            "12. EXPERIMENTS & DAILY SCHEDULING DIRECTIVE: Create habits/sprints via create_experiment. Organize daily focus into atomic blocks (~25–30 мин) by priority. NEVER invent fake clock timestamps (e.g. 09:30–10:00); specify exact times ONLY if explicitly defined in a task/event. MANDATORY ON-DEMAND SCHEDULING TOOL-CHAINING: When asked to construct, plan, or adjust a daily schedule on-demand in chat, ALWAYS invoke `get_active_rules(domain='productivity')`, `list_tasks(status='todo')`, and `get_active_experiments()` BEFORE producing the final schedule. Apply all retrieved axioms strictly.",
            "13. LIFE PRINCIPLES & PRODUCTIVITY AXIOMS DIRECTIVE:\n"
            "   - RULE QUERIES: When the user asks about their rules, axioms, principles, or habits (e.g. 'какие у меня есть правила?'), ALWAYS invoke `get_active_rules()` and read system rules to present the 4 core rules of systematicity and all active domain axioms.\n"
